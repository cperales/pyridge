...
def test_generalized_boosting_ridge():
    ...
    value_dict = check_algorithm(folder='data',
                                 dataset='iris',
                                 algorithm='GeneralizedBRELM',
                                 hyperparameter=hyperparameter_boost,
                                 metric_list=['accuracy', 'rmse'])


def test_generalized_boosting_ridge_regression():
    hyperparameter_boost = {'activation': ['sigmoid'],
                            'reg': [10 ** i for i in range(-1, 2)],
                            'hidden_neurons': [10],
                            'size': [5]}
...
if __name__ == '__main__':
    create_logger('INFO')
    test_generalized_boosting_ridge()
    test_generalized_boosting_ridge_regression()
