...
def multi_test():
    # Reading JSON
    with open('config/NELM_multiprueba.json', 'r') as cfg:
        config_options = json.load(cfg)
...
def run_test(config_test):
    training_file_name = os.path.join(config_test['Data']['folder'],
                                      config_test['Data']['trainingDataset'])
    ...
    testing_file_name = os.path.join(config_test['Data']['folder'],
                                     config_test['Data']['testingDataset'])
    ...
    # Saving classifier
    save_classifier(clf, 'NELM_newthyroid.clf')
    ...
    try:
        prof.dump_stats('profile/NELM_newthyroid.prof')
